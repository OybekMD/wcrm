...
func TestApi(t *testing.T) {
	...
	require.Equal(t, "testemail@gmail.com", user.Email)
	require.Equal(t, int64(17), user.Age)
	require.Equal(t, "Oybek", user.FirstName)
	require.Equal(t, "md", user.Username)
	require.Equal(t, "oybekpro", user.Password)
	...
	require.Equal(t, product.Description, "Oybek's Description")
	require.Equal(t, product.Name, "Oybek's Product")
